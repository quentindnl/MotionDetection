...
public class Detection {
    ...
    //Detection limit = 1 meters
    public func detectRunningDown() {
        if ( (abs(self.accX)>2 || abs(self.accY)>2 || abs(self.accZ)>2) && self.deltaAltitude<(-1) ) {
    ...
    public func detectRunningUpDown() {
        if ( isRunningUpDetected==true && abs(self.altitude)<1) {
    ...
    public func detectRunningDownUp() {
        if ( isRunningDownDetected==true && abs(self.altitude)<(-1)) {
    ...
    public func bicepCurlCount() {
        ...
        //A bicep curl is detected if acceleration max and min > 1G, counter++
